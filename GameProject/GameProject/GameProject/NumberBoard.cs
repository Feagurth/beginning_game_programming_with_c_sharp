    ...
    class NumberBoard
    {
            ...
            int correctNumber, SoundBank soundBank)
        {
            ...
            for (int i = 0; i < NUM_ROWS; i++)
            {
                ...
                {
                    // Step 65 Passing the sound bank to the NumberTile constructor
                    tiles[i, j] = new NumberTile(contentManager, CalculateTileCenter(i, j), tileSideLength,
                        ((i * NUM_ROWS) + (j + 1)), correctNumber, soundBank);
                }
            }
        }
        ...
        public bool Update(GameTime gameTime, MouseState mouse)
        {
            ...
            for (int i = 0; i < NUM_ROWS; i++)
            {
                for (int j = 0; j < NUM_COLUMNS; j++)
                {
                    // Step 52 Declaring a boolean variable to store the update result of the tiles  and it with the result of the update
                    bool numberGuessed = tiles[i, j].Update(gameTime, mouse);

                    // If the update is true, we inmediatly return true
                    if (numberGuessed)
                    {
